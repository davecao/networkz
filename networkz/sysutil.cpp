...
//  if (NZ_STAT(avx2_present))
//    return NZ_ATTRIB_ARCH_AVX2;
//  else if (NZ_STAT(avx_present))
//    return NZ_ATTRIB_ARCH_AVX;
//  else if (NZ_STAT(sse3_present))
//    return NZ_ATTRIB_ARCH_SSE;
//  else
//    return NZ_ATTRIB_ARCH_CPU;
//}
...
std::string sysutil_realpath(const std::string& path)
{
  ...
  else
  {
    ...
    {
      case EACCES:
        throw std::ios_base::failure("Can't access file: " + path);
        break;
      case ENOENT:
        throw std::ios_base::failure("File doesn't exist: " + path);
        break;
      case ELOOP:
      case ENAMETOOLONG:
        throw std::ios_base::failure("Path too long or too many symlinks: " + path);
        break;
      default:
        throw std::ios_base::failure("Unknown I/O error: " + path);
    }
  }
}
...
{
  return !sysutil_dir_exists(fname) &&
              access(fname.c_str(), access_mode) == 0;
}
